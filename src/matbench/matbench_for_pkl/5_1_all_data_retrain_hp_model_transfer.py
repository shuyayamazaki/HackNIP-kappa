#!/usr/bin/env python3
"""
Retrain MODNet using stored hyperparameters on the full dataset (train + test).

Given a metadata.json produced by opt_hp_modnet_from_supercells.py (alongside a saved
trial model), this script reuses the stored hyperparameters and layer selection to train
new MODNet models on other dataset slugs using *all* available data (both the train and
test splits).
"""
...
def train_all_data(
    ...
    y_all: np.ndarray,
    target_name: str,
) -> Tuple[MODNetModel, Dict[str, float], np.ndarray]:
    """Train a MODNet model using stored hyperparameters on the full dataset."""
    ...
    md_all = make_moddata(X_all[:, :n_features], y_all, target_name)
    ...
    all_pred = flatten_predictions(model.predict(md_all, remap_out_of_bounds=False))
    metrics = {
        "all_mae": float(mean_absolute_error(y_all, all_pred)),
        "all_r2": float(r2_score(y_all, all_pred)),
    }
...
def save_all_predictions(
    ...
    """Append train+test predictions (with original split labels) to a CSV."""
    path.parent.mkdir(parents=True, exist_ok=True)
    write_header = not path.exists()
    with open(path, "a", encoding="utf-8", newline="") as csv_fp:
        writer = csv.writer(csv_fp)
        if write_header:
            writer.writerow(["slug", "split", "index", "target", "prediction"])

        # Train portion
        for idx, (target, pred) in enumerate(zip(y_train, y_pred_all[: len(y_train)])):
            writer.writerow([slug, "train", idx, float(target), float(pred)])

        # Test portion
        offset = len(y_train)
        for idx, (target, pred) in enumerate(zip(y_test, y_pred_all[len(y_train) :])):
            writer.writerow([slug, "test", idx + offset, float(target), float(pred)])


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Retrain MODNet using best hyperparameters on full train+test data."
    )
    parser.add_argument(
        "--metadata-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        "(default: <results_modnet>/hp_transfer_retrain_all_data/<meta_stem>_<timestamp>).",
    )
    parser.add_argument(
        "--save-models",
        action="store_true",
    ...
    if output_dir is None:
        output_dir = RESULTS_DIR / "hp_transfer_retrain_all_data" / f"{meta_stem}_{timestamp}"
    ...
    for slug in target_slugs:
        ...
        train_bundle, _ = load_feature_bundle(slug, feature_key, args.train_split)
        test_bundle, _ = load_feature_bundle(slug, feature_key, args.test_split)
        ...
        # Concatenate train and test to form the full dataset.
        ...
        model, metrics, all_pred = train_all_data(
            params=hyperparams,
            X_all=X_all,
            y_all=y_all,
            target_name=target_name,
        ...
        # Save per-sample predictions (train + test) with split labels for inspection.
        all_pred_path = output_dir / "all_predictions.csv"
        ...
        print(
            f"[RESULT] Slug {slug} | All MAE {metrics['all_mae']:.6f} | "
            f"All R2 {metrics['all_r2']:.6f}"
        )

        if args.save_models:
            model_path = models_dir / f"{slug}_{feature_key}_{MLIP}_l{layer}_all.modnet"
            model.save(str(model_path))
            print(f"[INFO] Saved retrained model: {model_path}")

    if metrics_rows:
        metrics_path = output_dir / "transfer_retrain_all_data_metrics.csv"
