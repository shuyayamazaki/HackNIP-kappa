# Original code from: https://github.com/PV-Lab/ZoMBI
# This code is revised by Dr. Yang Jeong Park, Prof. Ju Li group at MIT.
...
class ZoMBI:
    def __init__(self, dataset_X, dataset_fX, fX_model, BO, nregular, activations, memory, forward, ensemble, batch=1, 
        ...
        # GP model
        self.GP = GaussianProcessRegressor(kernel = ConstantKernel(1.0) * Matern(
            length_scale = 5, nu = 1) * 0.5, n_restarts_optimizer = 30,
            alpha = 0.0002, normalize_y = False)

    def labeling(self, idx):
        '''
        Labeling function for the ZoMBI optimization procedure.
        :param idx:     The indices of the points to label
        :return:        The labeled points
        '''
        return self.fX_model(self.dataset_X[idx])
    ...
    def activate_zombi(self, current_act, total_act):
        ...
        fX_bounded = -1 * self.GP.predict(X_bounded)
        ...
        for n in range(self.forward):
            ...
            ac_value = self.BO(X=bounded_norm, GP_model=self.GP, n=n, fX_best=self.fX_min_i[-1],
                            fX_best_min=self.fX_min_i, xi=self.xi, beta=self.beta, beta_ab=self.beta_ab,
                            eta=self.eta, ratio=self.ratio, decay=self.decay)
            ...
            # Ensure no overlap between current and previous indices
            while np.any(np.isin(idx, idx_old)):
                j += 1
                idx = np.argsort(ac_value)[j:j + self.batch]

            # Update X_bounded with new points
            X_bounded = np.vstack([X_new, bounded_norm[idx]])
            fX_bounded_new = -1 * self.GP.predict(bounded_norm[idx])
    ...
    def optimize(self, X_initial, fX_initial, plot_f=True):
        ...
        :param fX_initial:      An (n,) array of evaluated initialization points f(X)
        ...
        '''
        # Initialize global visited indices
        if not hasattr(self, "visited_indices"):
            self.visited_indices = set()

        for e in range(self.ensemble):
            self.e = e
            fX_best_list = []
            idx_old = np.array([])  # Initialize as an empty array
            inc = 0.1
            ...
            for n in range(self.nregular):
                ...
                ac_value = self.BO(X=self.dataset_X, GP_model=self.GP, n=n, fX_best=fX_best, fX_best_min=fX_best_min,
                                xi=self.xi, beta=self.beta, beta_ab=self.beta_ab, eta=self.eta,
                                ratio=self.ratio, decay=self.decay)
                self.compute_i.append(time.time() - start)

                # Select indices for the new batch
                sorted_indices = np.argsort(ac_value)
                unvisited_indices = [idx for idx in sorted_indices if idx not in self.visited_indices]

                idx = unvisited_indices[:self.batch]  # Select the batch of unvisited indices

                # Update visited indices
                self.visited_indices.update(idx)
                idx = np.array(idx)  # Ensure numpy array for consistency
                print(len(self.visited_indices))

                # Update dataset with new points
                X_initial_i = np.vstack([X_new, self.dataset_X[idx]])  # Grab predicted x-values from dataset
                fX_initial_i = np.append(fX_new, self.labeling(idx))  # Grab predicted y-values from dataset
            
            # Store optimized results for the current ensemble
            self.X_i = X_initial_i
            self.fX_i = fX_initial_i
            self.fX_min_i = np.minimum.accumulate(-1 * self.fX_i)

            # ZoMBI activation phase
            ...
            # Combine results across ensembles
            if e == 0:
                self.X = self.X_i
                self.fX = self.fX_i
