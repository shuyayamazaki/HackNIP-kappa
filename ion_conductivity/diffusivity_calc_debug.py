...
EXAMPLE_DIR = pathlib.Path("orb_call").resolve()
INPUT_DIR = EXAMPLE_DIR / "input"
OUTPUT_DIR = EXAMPLE_DIR / "output"


def run_md_simulation(temperature, index, calculator='pfp', device=0, debug=False):
    """
    ...
    Returns:
        float: Diffusion coefficient (D) in cm^2/sec (10000 * A^2/psec).
    """
    s_time = perf_counter()
    ...
    if calculator == 'pfp':
        ...
        estimator = Estimator(model_version="v7.0.0", calc_mode=EstimatorCalcMode.CRYSTAL)
        calculator = ASECalculator(estimator)
    elif calculator == 'orb':
        ### ORB model
        from orb_models.forcefield import pretrained
        from orb_models.forcefield.calculator import ORBCalculator
        device=f"cuda:{device}" # or device="cuda"
    ...
    elif calculator == 'eqV2':
        ### eqV2 model
        # available cuda device is 1
        import os
        os.environ["CUDA_VISIBLE_DEVICES"] = f"{device}"
        from fairchem.core import OCPCalculator
        calculator = OCPCalculator(checkpoint_path="/home/lucky/Projects/ion_conductivity/eqV2_86M_omat_mp_salex.pt", cpu=False) # Path to downloaded checkpoint
    ...
    if debug:
        ...
        dyn.run(200_000) # 4 ns
    proctime = perf_counter() - s_time

    # Analyze trajectory for diffusion coefficient
    trajectory = read(traj_file, index="::") if debug else read(traj_file, index="::5")
